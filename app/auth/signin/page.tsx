...
function friendlyAuthError(raw: string): { message: string; hint?: string } {
  ...
  if (msg.includes("user not found"))
    return { message: "No account found with this email. Try signing up." };
    ...
    <div className="flex min-h-screen w-full">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-[440px]">
          ...
          {callbackError === "callback_failed" && !errorMsg && (
          ...
          {noticeParam === "account_exists" && !errorMsg && step === "email" && (
            <div className="mb-6 p-4 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300 text-sm">
              An account already exists with that email. Sign in below.
            </div>
          )}

          {/* Password reset success notice */}
          {noticeParam === "password_reset" && !errorMsg && step === "email" && (
          ...
          {step === "email" ? (
            ...
            <>
              ...
              >
                {isGoogleLoading ? (
                  <FontAwesomeIcon icon={faSpinner} className="mr-2.5 text-base animate-spin" />
                ) : (
              ...
              <form onSubmit={handleEmailSubmit} className="space-y-4">
                ...
                <Button
                  ...
                  disabled={isLoading}
                  aria-label={isLoading ? "Looking up your account" : "Continue to password"}
                >
                  {isLoading ? (
                    <>
              ...
              {accounts.length >= 1 && selectedWorkspaceId && (
                <p className="text-sm text-muted-foreground mb-6 flex items-center gap-2">
                  ...
                  <span className="font-semibold text-foreground">
                    {accounts.find((a) => a.workspaceId === selectedWorkspaceId)?.workspaceName}
                  </span>
                </p>
              )}
